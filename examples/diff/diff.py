...
class DiffLibWrapper(object):
    ...
    def unified_diff(self, lines=3):
        return difflib.unified_diff(
            self.fromlines, self.tolines, self._fromfile, self._tofile, self.fromdate, self.todate, n=lines
        )
    ...
    def make_file(self, context=False, lines=3):
        return difflib.HtmlDiff().make_file(
            self.fromlines, self.tolines, self._fromfile, self._tofile, context=context, numlines=lines
        )
...
def main():
    afire.Fire(DiffLibWrapper, name="diff")
