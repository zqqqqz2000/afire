...
from afire import completion
from afire import custom_descriptions
from afire import decorators
from afire import docstrings
from afire import formatting
from afire import inspectutils
...
def _ArgsAndFlagsSections(info, spec, metadata):
    ...
    positional_flag_items = [
        _CreateFlagItem(flag, docstring_info, spec, required=False, short_arg=flag[0] in unique_short_args)
...
def _GetArgsAndFlagsString(spec, metadata):
    ...
    if args_with_no_defaults:
        if accepts_positional_args:
            ...
            arg_strings = [
                "--{arg}={arg_upper}".format(arg=arg, arg_upper=formatting.Underline(arg.upper()))
...
def _CreateOutputSection(name, content):
    return """{name}
{content}""".format(
        name=formatting.Bold(name), content=formatting.Indent(content, SECTION_INDENTATION)
    )
...
def _NewChoicesSection(name, choices):
    return _CreateItem(
        "{name} is one of the following:".format(name=formatting.Bold(formatting.Underline(name))),
        "\n" + "\n\n".join(choices),
        indent=1,
    )
...
For detailed information on this command, run:
  {help_command}"""
    ...
    return output_template.format(
        continued_command=continued_command, availability_lines="".join(availability_lines), help_command=help_command
    )
    ...
    for action_group in actions_grouped_by_kind:
        if action_group.members:
            availability_line = _CreateAvailabilityLine(
                header="available {plural}:".format(plural=action_group.plural), items=action_group.names
            )
...
def _GetCallableAvailabilityLines(spec):
    ...
    optional_flags = [
        ("--" + flag) for flag in itertools.chain(args_with_defaults, _KeywordOnlyArguments(spec, required=False))
    ]
